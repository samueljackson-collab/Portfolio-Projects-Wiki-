...
**Decision:** We will use a combination of **SonarQube for SAST**, **OWASP ZAP for DAST**, and **Trivy for SCA and container scanning**.\n\n**Consequences:**\n*   **Pros:** This combination provides broad coverage across different vulnerability types. All chosen tools are open-source leaders in their respective categories with strong community support and integration capabilities with GitHub Actions.\n*   **Cons:** Requires managing the configuration and maintenance of three separate tools. There might be some overlap in vulnerability findings, requiring careful tuning of policies to avoid redundant alerts.`, "threatModel": "### STRIDE Threat Model\n\n*   **Spoofing:** A malicious actor spoofs the identity of a scanner (e.g., SonarQube) to report false results. **Mitigation:** Use webhooks with shared secrets for integrations. Run scanners on trusted, self-hosted runners where possible.\n*   **Tampering:** An attacker modifies the scanner configuration to disable certain security checks. **Mitigation:** Store scanner configurations as code in the Git repository with branch protection rules. Regularly audit the configuration.\n*   **Repudiation:** A developer denies that a vulnerability was present at the time of a scan. **Mitigation:** The CI/CD pipeline logs provide an immutable, timestamped record of every scan and its results for each commit.\n*   **Information Disclosure:** Scan results, which may contain details about vulnerabilities, are exposed to unauthorized users. **Mitigation:** Restrict access to the CI/CD logs and the security dashboard (e.g., SonarQube UI) to authorized personnel.\n*   **Denial of Service:** A DAST scan overloads and crashes the staging environment. **Mitigation:** Configure the DAST scanner to run with appropriate intensity levels. Run scans during off-peak hours if necessary.\n*   **Elevation of Privilege:** A compromised scanner or a vulnerability in the pipeline itself gains elevated permissions. **Mitigation:** Run each stage of the CI/CD pipeline in a separate, isolated environment (e.g., container) with the minimum required permissions. Avoid using a single, overly permissive credential for the entire pipeline.", "cicdWorkflow": { "name": "security-scan.yml", "path": ".github/workflows/security-scan.yml", "content": "name: Security Scan Pipeline\n\non:\n  push:\n    branches: [ main ]\n  pull_request:\n\njobs:\n  security-scans:\n    name: 'Run Security Scans'\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n\n      - name: Run SonarQube SAST Scan\n        uses: sonarsource/sonarqube-scan-action@master\n        env:\n          SONAR_TOKEN: ${{ secrets.SONAR_TOKEN }}\n          SONAR_HOST_URL: ${{ secrets.SONAR_HOST_URL }}\n\n      - name: Run Trivy Dependency Scan\n        uses: aquasecurity/trivy-action@master\n        with:\n          scan-type: 'fs'\n          scan-ref: '.'\n          severity: 'CRITICAL,HIGH'\n\n      - name: Build Docker image for DAST\n        run: docker build -t my-app-for-dast .\n\n      - name: Run OWASP ZAP DAST Scan\n        uses: zaproxy/action-baseline@v0.7.0\n        with:\n          target: 'http://localhost:8080' # Assuming the app runs on this port\n          docker_name: 'my-app-for-dast'\n" } },
    ...
    { "id": 24, "name": "Portfolio Report Generator", "slug": "report-generator", "description": "Automated report generation system using Jinja2 and WeasyPrint.", "status": "Planned", "completion_percentage": 15, "tags": ["automation", "reporting", "python"], "github_path": "projects/24-report-generator", "technologies": ["Python", "Jinja2", "WeasyPrint", "APScheduler"], "features": ["Scheduled generation", "Email delivery", "Historical trending", "PDF/HTML output"], "key_takeaways": ["Automating reporting saves significant manual effort and ensures consistency.", "Separating data gathering, templating (Jinja2), and rendering (WeasyPrint) creates a flexible and maintainable pipeline.", "A good report is not just data; it's data presented with context and clarity."] },
    { "id": 25, "name": "Portfolio Website", "slug": "portfolio-website", "description": "Static documentation portal generated with VitePress.", "status": "Substantial", "completion_percentage": 65, "tags": ["web", "vitepress", "documentation", "vue"], "github_path": "projects/25-portfolio-website", "technologies": ["VitePress", "Vue.js", "Node.js", "GitHub Pages"], "features": ["Project showcase", "Automated deployment", "Responsive design", "Search functionality"], "key_takeaways": ["Static site generators offer excellent performance, security, and low hosting costs.", "Treating documentation as code (Docs-as-Code) by keeping it in Git alongside the project encourages updates and collaboration.", "A well-structured information architecture is crucial for a good user experience on a documentation site."] },
